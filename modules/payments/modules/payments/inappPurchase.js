...
const InappPurchase = () => {
  ...
  const [appleIapProductIds, setAppleIapProductIds] = useState([]);

  const productIds = Platform.select({
    ios: appleIapProductIds,
    android: [
  ...
  const getAppleIAPProductIds = async () => {
    const data = await fetchAppleIAPProducts();
    if (data) {
      console.log("data", data);
      data.forEach((obj) => {
        appleIapProductIds.push(obj.product_id);
      });
      console.log("value", appleIapProductIds);
      getProducts();
    }
  };
