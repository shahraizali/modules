...
from modules.payments.payments.models import StripeUserProfile
...
class StripeSubscriptionService:
    ...
    def get_products(cls):
        ...
        """
            get_products returns an object with a data property that contains an array of up to limit products. 
    ...
    def get_product_prices(cls, p_id):
        """
            get_product_prices returns an object with a data property that contains an array of up to limit prices.
            If no more prices are available, the resulting array will be empty.
    ...
    @classmethod
    def get_price_details(cls, p_id):
        """
    ...
    def get_product_details(cls, p_id):
        """
        get_product_details Retrieves the details of an existing product.
    ...
    def create_subscription(cls, cust_id, price_id):
        """
        create_subscription creates a new subscription on an existing customer.
    ...
    def update_subscription(cls, sub_id, price_id):
        """
        update_subscription updates an existing subscription to match the specified parameters.
    ...
    def cancel_subscription(cls, sub_id):
        """
        cancel_subscription cancels a customer’s subscription immediately. Returns the canceled Subscription object.
        Its subscription status will be set to cancel.
    ...
    def create_invoice_intent_sheet(cls, cust_id, price_id, behavior='default_incomplete'):
        """
        create_invoice_intent_sheet creates a new incomplete status subscription and respective intent to capture payment.
    ...
    def validate_webhook_payload(cls, payload):
        """
        Validates an event occurred in the app is by the stripe.
    ...
    @classmethod
    def get_event_from_webhook(cls, payload):
    ...
    def end_subscription(cls, sub):
        ...
        from modules.payments.payments.models import UserSubscription
        customer = sub.customer
        sub, created = UserSubscription.objects.get_or_create(
            user=StripeUserProfile.objects.get(stripe_cus_id=customer).user)
        sub.is_active = False
        sub.save()
    ...
    def modify_subscription(cls, sub):
        # You can use an email or a mobile push notification to customer to let him know to pay
        from modules.payments.payments.models import UserSubscription, SubscriptionPlan
        customer = sub.customer
        s, created = UserSubscription.objects.get_or_create(
            user=StripeUserProfile.objects.get(stripe_cus_id=customer).user)
        s.is_active = True
    ...
    def customer_subscribed(cls, obj):
        from modules.payments.payments.models import UserSubscription, SubscriptionPlan, UserSubscriptionHistory
    ...
    def log_stripe_event(cls, event):
        ...
        """
        from modules.payments.payments.models import StripeWebhookLog
        try:
